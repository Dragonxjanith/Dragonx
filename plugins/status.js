    ...
    Trex.addrex({pattern: 'down$', fromMe: true }, (async (message, match) => {    
        ...
        if (message.reply_message.video === false && message.reply_message.image) {
            ffmpeg(location)
                .save('stat.png')
                .on('end', async () => {
                    await message.client.sendMessage(message.client.user.jid,fs.readFileSync('stat.png'), MessageType.image,{mimetype: Mimetype.png, caption: '⎝🛡️  ᗪᖇ𝙰𝙶𝚘𝚗 Ӽ   🛡️⎠' ,quoted: message.data});
            });
        return 
        }
